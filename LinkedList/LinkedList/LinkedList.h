...
// Checks to see what the maximum value is in the linked list
template <typename T>
...
// Checks to see what the minimum value is in the linked list
template <typename T>
Node<T>* LinkedList<T>::minimum()
{
	...
	while (current != nullptr)
	{
		if (current->get_data() < min_node->get_data())
		{
			min_node = current;
		}
		current = current->get_next();
